...
public class Utils {
     ...
     * @return
     */
    public static Librairie  triLivreParScore(Librairie pLibraire) {
        Livre temp;
        for (int i = pLibraire.getListeLivres().size() - 1; i >= 1; i--) {
            for (int j = 0; j < i; j++)
                if (pLibraire.getListeLivres().get(j).getScore() < pLibraire.getListeLivres().get(j + 1).getScore()) {
                    ...
                    pLibraire.getListeLivres().set(j, temp);
                }
        }
     ...
     * @return
     */
    public static Librairie calculerOuverture(List<Librairie> pLibrairiesFermees, int nbJoursRestants) {
        ...
        if (null != pLibrairiesFermees && !pLibrairiesFermees.isEmpty()
            ...
            for (Librairie pLibrairieEnCours : pLibrairiesFermees) {
            	pLibrairieEnCours=triLivreParScore(pLibrairieEnCours);
                ...
                for (int i = 0; i<LivreATraiter-1; i++) {
                	if(i<pLibrairieEnCours.getListeLivres().size()) {
